  ...
  constructor(options) {
    super({
      ...
      float fresnelFunc(vec3 viewDirection, vec3 worldNormal) {
        return pow(1.0 - dot(viewDirection, worldNormal), 2.0);
      }
      void main() {
        vec2 uv = gl_FragCoord.xy / resolution;
        vec3 normal = worldNormal * (1.0 - 0.7) - texture2D(backfaceMap, uv).rgb * .3;
        vec4 color = texture2D(envMap, uv += refract(viewDirection, normal, 1.0/1.5).xy);
        float fresnel = fresnelFunc(viewDirection, normal);
        gl_FragColor = vec4(mix(color.rgb, vec3(0.8, 0.55, 0.65), fresnel), 1.0);
      }`,
      uniforms: {
